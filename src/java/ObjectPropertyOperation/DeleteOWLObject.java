 ...
 * @author amar_
 */
public class DeleteOWLObject extends HttpServlet {
...
for (OWLObjectProperty obj : ontology.getObjectPropertiesInSignature()) {
    if (obj.getIRI().getFragment().equals(objName)){
               obj.accept(remover);
...
//response.getWriter().println(ontclass);
    ...
    response.sendRedirect(request.getContextPath() + "/MainOWLObject?name="+objName);
